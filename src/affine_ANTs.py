...
parser.add_argument("-b", "--base_image", help="Select the modality for Co-rigid", default="FLAIR.nii.gz")
...
parser.add_argument("-i", "--read_dir", help="Input path of the patient directory", default="input")
...
print("\n##### Affine(ANTs) #####\n")
def affine(src_path, dst_path, ref_path,save_path_affine,src_path_mask, dst_path_mask,mode="modality",thr=0.9):
    file_name=""
    dis_path_only=""
    fl=False
    ...
    if(mode=="base_image"):
        ...
        "--transform","Affine[0.1]","--metric","MI[",
...
def main(input_path, output_path, reference_path,save_path_affine,src_path_mask, dst_path_mask,mode="modality"):
    print("Applying Affine(ANTs) on: ", input_path)
    try:
        affine(input_path, output_path, reference_path,save_path_affine,src_path_mask, dst_path_mask,mode)
    except RuntimeError:
        print("\tFalied to apply Affine(ANTs) on: ", input_path)
