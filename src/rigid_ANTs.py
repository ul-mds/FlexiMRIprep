...
print("\n##### Rigid(ANTs) #####\n")
def rigid(src_path, dst_path, ref_path,save_path_affine,src_path_mask, dst_path_mask,mode="modality",thr=0.9):
    file_name=""
    dis_path_only=""
    fl=False
    ...
    if(mode=="base_image"):
        ...
        "--initial-moving-transform","[",ref_path,",",src_path,",","1","]", "--transform","Rigid[0.1]","--metric","MI[",
...
def main(input_path, output_path, reference_path,save_path_affine,src_path_mask, dst_path_mask,mode="modality"):
    print("Applying Rigid(ANTs) on: ", input_path)
    try:
        rigid(input_path, output_path, reference_path,save_path_affine,src_path_mask, dst_path_mask,mode)
    except RuntimeError:
        print("\tFalied to apply Rigid(ANTs) on: ", input_path)
